...
function crearLightbox(){
  ...
  // Crea el overlay una sola vez al cargar la página
  ...
  // Cerrar al hacer clic en el overlay o en el botón
...
// ← CAMBIO PRINCIPAL: ya no comparamos contraseñas aquí.
//   Las enviamos al servidor y él decide.
async function entrar(){
  ...
  // Aplicar rol devuelto por el servidor
  esAdmin       = data.rol === "admin";
  esRegistrador = data.rol === "admin" || data.rol === "registro";

  cerrarLogin();

  if(data.rol === "visitante") return;
  ...
  // Recargar para mostrar botones de borrar
  cargarRegistros();
  cargarArticulos();
  cargarProductos();
  cargarDiagramas();
  ...
  for(const r of data){
    ...
    lista.innerHTML += `
      <div class="card">
        ...
        ${r.imagen
          ? `<img src="${r.imagen}" class="imagenCard imagenZoom"
               onclick="abrirLightbox('${r.imagen}')"
  ...
  for(const a of data){
    ...
    const botonesAdmin = esAdmin
      ? `<div style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap;">
           <button type="button" class="btn-editar"
             onclick="mostrarFormEditar('${a._id}')">
             ✏️ Editar artículo
           </button>
           <button type="button" class="btn-borrar"
         ...
         <div id="form-editar-${a._id}" style="display:none; margin-top:14px;">
           <div class="form-editar-articulo">
             <label>Título:</label>
             <input type="text" id="edit-titulo-${a._id}"
               value="${a.titulo.replace(/"/g,'&quot;')}">
             <label>Contenido:</label>
             <textarea id="edit-contenido-${a._id}" rows="6">${a.contenido}</textarea>
             <label>Imagen (opcional — deja vacío para mantener la actual):</label>
             <input type="file" id="edit-imagen-${a._id}" accept="image/*">
             <div style="display:flex; gap:8px; margin-top:8px;">
               <button type="button" class="btn-guardar"
                 onclick="guardarEdicionArticulo('${a._id}')">
                 💾 Guardar cambios
               </button>
               <button type="button" style="background:#95a5a6;"
                 onclick="ocultarFormEditar('${a._id}')">
                 Cancelar
               </button>
             </div>
           </div>
    ...
    lista.innerHTML += `
      <div class="card" id="item-articulo-${a._id}">
        <h2 id="titulo-art-${a._id}">${a.titulo}</h2>
        <p class="texto-articulo" id="contenido-art-${a._id}">${a.contenido}</p>
        ${a.imagen
          ? `<img src="${a.imagen}" class="imagenCard imagenZoom"
               id="img-art-${a._id}"
               onclick="abrirLightbox('${a.imagen}')"
               title="Clic para ver en grande">`
          : `<span id="img-art-${a._id}"></span>`
        }
        ${botonesAdmin}
        <div class="seccion-comentarios">
          ${comentariosHTML}
...
async function guardarEdicionArticulo(id){
  ...
  const titulo =
  document.getElementById(`edit-titulo-${id}`).value.trim();

  const contenido =
  document.getElementById(`edit-contenido-${id}`).value.trim();
  ...
  if(data._id || data.ok){
    ...
    // Actualizar la tarjeta en pantalla sin recargar todo
    document.getElementById(`titulo-art-${id}`).textContent    = titulo;
    document.getElementById(`contenido-art-${id}`).textContent = contenido;

    if(imagenFile && data.imagen){
      const imgEl = document.getElementById(`img-art-${id}`);
      if(imgEl && imgEl.tagName === "IMG"){
        imgEl.src = data.imagen;
        imgEl.onclick = () => abrirLightbox(data.imagen);
      }
    }

    ocultarFormEditar(id);
  ...
  for(const p of data){
    ...
    lista.innerHTML += `
      <div class="card">
        ...
        ${p.imagen
          ? `<img src="${p.imagen}" class="imagenCard imagenZoom"
               onclick="abrirLightbox('${p.imagen}')"
               title="Clic para ver en grande">`
          : ""
        }
  ...
  for(const d of data){
    ...
    lista.innerHTML += `
      <div class="card" id="item-diagrama-${d._id}">
        <h2>${d.titulo}</h2>
        ${d.imagen
          ? `<img src="${d.imagen}" class="imagenCard imagenZoom"
               onclick="abrirLightbox('${d.imagen}')"
               title="Clic para ver en grande">`
          : ""
        }
        ${btnBorrar}
...
async function cargarTutoriales(){
  ...
  for(const t of data){
    ...
    // Intentar convertir a embed; si no es YouTube mostrar enlace normal
    const embedUrl = obtenerEmbedYoutube(t.link);
